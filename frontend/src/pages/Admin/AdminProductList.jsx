...
import { Loader2, Plus, Trash2, Edit3, X, CheckCircle2 } from 'lucide-react';
...
const Modal = ({ initial, onClose, onSave, saving, error }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="bg-[#0A0F1C] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
          ...
          <button onClick={onClose} className="text-textSecondary hover:text-textMain">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={submit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          {(err || error) && (
            <p className="text-red-400 text-xs font-bold bg-red-500/10 border border-red-500/20 px-4 py-2 rounded-xl">
              {err || error}
            </p>
          )}

          {fields.map(({ k, label, req, type = 'text', ph }) => (
            <div key={k}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-1 bg-primary hover:bg-primary/90 text-white py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {saving
                ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving...</>
                : <><CheckCircle2 className="h-4 w-4" /> {initial?._id ? 'Save Changes' : 'Create Product'}</>}
            </button>
          </div>
        </form>
      </div>
...
const AdminProductList = () => {
  ...
  const [modal, setModal]       = useState(null);  // null = closed, {} = new, {...product} = edit
  ...
  const handleSave = async (data) => {
    ...
    try {
      if (modal?._id) {
        await api.put(`/products/${modal._id}`, data);
  ...
  return (
    <div>
      {modal !== null && (
        <Modal
          initial={modal}
          onClose={() => { setModal(null); setActionErr(''); }}
          onSave={handleSave}
          saving={saving}
          error={actionErr}
        />
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-textSecondary text-sm mt-1">{products.length} products</p>
        </div>
        <button
          onClick={() => { setModal({}); setActionErr(''); }}
          className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all shadow-glow active:scale-95"
        >
      ...
      {successMsg && (
        <p className="text-green-400 text-xs font-bold bg-green-500/10 border border-green-500/20 px-4 py-2 rounded-xl mb-4 flex items-center gap-2">
          <CheckCircle2 className="h-4 w-4" /> {successMsg}
        </p>
      )}
      {actionErr && !modal && (
        <p className="text-red-400 text-xs font-bold bg-red-500/10 border border-red-500/20 px-4 py-2 rounded-xl mb-4">{actionErr}</p>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-textSecondary gap-3">
          <p className="text-sm font-bold">No products yet. Add your first one!</p>
        </div>
      ) : (
        <>
                  ...
                  <tr key={p._id} className="border-t border-white/5 hover:bg-white/5">
                    <td className="px-5 py-3">
                      {p.images?.[0]
                        ...
                        : <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-textSecondary/30 text-xs">N/A</div>}
                    </td>
                    <td className="px-5 py-3 font-bold text-textMain max-w-[160px] truncate">{p.name}</td>
                    <td className="px-5 py-3 text-textSecondary text-xs font-bold uppercase">{p.category}</td>
                    <td className="px-5 py-3 font-black text-accent">{symbol}{p.price}</td>
                    <td className="px-5 py-3">
                      <span className={`text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-widest ${
                        p.countInStock > 0 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
                      }`}>
                    ...
                    <td className="px-5 py-3">
                      {p.featured
                        ? <span className="text-[10px] font-black px-2 py-1 rounded-full bg-accent/10 text-accent uppercase tracking-widest">Yes</span>
                        : <span className="text-[10px] text-textSecondary/30 font-bold">—</span>}
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => { setModal({ ...p, images: p.images?.join(', ') || '' }); setActionErr(''); }}
